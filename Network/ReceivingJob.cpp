...
namespace Network
{
	...
	auto ReceivingJob::working(void) -> std::expected<void, std::string>
	{
		if (receiving_callback_ == nullptr)
		{
			return std::unexpected("cannot complete ReceivingJob with null callback");
		}

		auto data = get_data();
		if (data.empty())
		{
			return std::unexpected("cannot complete ReceivingJob with null data");
		}
		...
		if (mode_data.size() != 1)
		{
			return std::unexpected("cannot complete ReceivingJob with null data mode");
		}
		...
		if (array_data.empty())
		{
			return std::unexpected("cannot complete ReceivingJob with empty data");
		}
