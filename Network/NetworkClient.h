...
namespace Network
{
	...
	public:
		NetworkClient(const std::string& client_id,
					  uint16_t high_priority_count = 3,
					  uint16_t normal_priority_count = 3,
					  uint16_t low_priority_count = 3);
		...
		auto auto_pong(bool enable) -> void { auto_pong_enabled_ = enable; }

		auto start(const std::string& ip, uint16_t port, size_t socket_buffer_size) -> bool;
		auto wait_stop(uint32_t seconds = 0) -> void;
		auto stop(void) -> void;

		auto register_key(const std::string& key) -> void;

		auto received_connection_callback(const std::function<std::expected<void, std::string>(bool, bool)>& callback) -> void;
		auto received_binary_callback(const std::function<std::expected<void, std::string>(const std::string&, const std::vector<uint8_t>&)>& callback)
			-> void;
		auto received_message_callback(const std::function<std::expected<void, std::string>(const std::string&)>& callback) -> void;
		auto received_file_callback(const std::function<std::expected<void, std::string>(const std::string&, const std::vector<uint8_t>&)>& callback) -> void;
	...
	private:
		...
		auto create_socket(const std::string& ip, uint16_t port) -> bool;
	...
	private:
		...
		std::function<std::expected<void, std::string>(bool, bool)> received_connection_callback_;
		std::function<std::expected<void, std::string>(const std::string&)> received_message_callback_;
		std::function<std::expected<void, std::string>(const std::string&, const std::vector<uint8_t>&)> received_file_callback_;
		std::function<std::expected<void, std::string>(const std::string&, const std::vector<uint8_t>&)> received_binary_callback_;
		std::function<std::expected<void, std::string>(const std::vector<std::string>&, const std::vector<std::pair<std::string, std::string>>&)>
