...
namespace Thread
{
	...
	public:
		...
		auto clear(JobPriorities priority) -> void;
		...
		auto notify_callback(const std::function<void(JobPriorities)>& callback) -> void;
	...
	private:
		...
		std::function<void(JobPriorities)> notify_callback_;
