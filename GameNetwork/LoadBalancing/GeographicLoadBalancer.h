...
namespace GameNetwork
{
    ...
    public:
        ...
        auto select_optimal_server(const ClientGeoInfo& client_info, 
                                  float distance_weight = 0.6f,
                                  float load_weight = 0.4f) -> std::optional<std::string>;

        // Regional management
        auto get_servers_in_region(const std::string& region) -> std::vector<std::string>;
        auto set_region_preference(const std::string& client_region, 
    ...
    private:
        ...
        auto calculate_server_score(const ServerGeoInfo& server, const ClientGeoInfo& client,
    ...
    private:
        ...
        // Configuration
        double max_distance_km_{10000.0}; // 10,000 km default
        bool enable_distant_failover_{true};
        
        // Constants for latency estimation
