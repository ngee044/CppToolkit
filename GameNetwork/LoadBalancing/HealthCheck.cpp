...
namespace GameNetwork
{
	namespace LoadBalancing
	{
		...
		TcpHealthChecker::TcpHealthChecker(const std::string& host, uint16_t port, std::chrono::seconds timeout)
		...
		auto TcpHealthChecker::check_health() -> std::tuple<bool, HealthCheckResult>
		{
			...
			try {
				...
				if (connected && socket.is_open()) {
					...
					result.details = fmt::format("TCP connection successful to {}:{}", host_, port_);
					
					using namespace Utilities;
					Logger::handle().write(LogTypes::Information,
						fmt::format("Health check passed for {}:{} (response time: {}ms)", host_, port_, result.response_time.count()));
					
					return {true, result};
				} else {
					result.status = HealthStatus::Unhealthy;
					result.details = fmt::format("TCP connection failed to {}:{}", host_, port_);
					...
					Logger::handle().write(LogTypes::Error,
						fmt::format("Health check failed for {}:{} - {}", host_, port_, result.details));
				...
				result.response_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
				result.status = HealthStatus::Unhealthy;
				result.details = fmt::format("TCP health check exception: {}", e.what());

				using namespace Utilities;
				Logger::handle().write(LogTypes::Error,
					fmt::format("Health check exception for {}:{} - {}", host_, port_, e.what()));
		...
		ServerHealthMonitor::ServerHealthMonitor(const std::string& server_id, const HealthCheckConfig& config)
		...
		auto ServerHealthMonitor::start() -> void
		{
			...
			Logger::handle().write(LogTypes::Information, fmt::format("Health monitor started for server: {}", server_id_));
		}
		...
		auto ServerHealthMonitor::stop() -> void
		{
			...
			Logger::handle().write(LogTypes::Information, fmt::format("Health monitor stopped for server: {}", server_id_));
		}
		...
		auto ServerHealthMonitor::perform_health_checks() -> HealthCheckResult
		{
			...
			for (const auto& checker : health_checkers_)
			{
				...
				check_details.push_back(fmt::format("{}: {}", checker->get_check_type(), result.details));
			...
			// Combine details
			combined_result.details = fmt::format("{} checks passed", health_checkers_.size() - failed_checks);
